...
export default definePluginEntry({
    ...
    register(api) {
        ...
        function resolveCliBinary() {
            if (process.env.INVESTORCLAW_CLI)
                return process.env.INVESTORCLAW_CLI;
        ...
        async function run(command, args = [], timeoutMs = ANALYSIS_TIMEOUT_MS) {
            try {
                ...
                const { stdout, stderr } = await execFileAsync(cliBinary, [command, ...args], { env, cwd: skillRoot, timeout: timeoutMs });
        ...
        api.registerTool({
            ...
            label: "Export Report",
            description: "Export portfolio data to CSV or Excel. Requires the holdings snapshot from " +
        ...
        api.registerTool({
            ...
            description: "Inject InvestorClaw data-integrity and financial-advice guardrail rules into the " +

...
2. Then ask Claude:
   ...
   Claude runs the skill's setup, which **pulls `ghcr.io/argonautsystems/ic-engine:4.7.1-cpu`**
...
```bash
docker pull ghcr.io/argonautsystems/ic-engine:4.7.1-cpu
```
...
docker run --rm \
  ...
  ghcr.io/argonautsystems/ic-engine:4.7.1-cpu holdings
```
...
docker run --rm -p 18090:8090 -p 18092:8092 \
  -e INVESTORCLAW_PRICE_PROVIDER=massive \
  -v "$PWD/ic-data:/data" \
  ghcr.io/argonautsystems/ic-engine:4.7.1-cpu

...
The image is public (`ghcr.io/argonautsystems/ic-engine:4.5.0-cpu`); no registry
...
```bash
docker pull ghcr.io/argonautsystems/ic-engine:4.5.0-cpu
```
...
docker run --rm \
  ...
  ghcr.io/argonautsystems/ic-engine:4.5.0-cpu holdings
```
...
docker run --rm -p 18090:8090 -p 18092:8092 \
  -e INVESTORCLAW_PRICE_PROVIDER=<provider> \
  -v "$PWD/ic-data:/data" \
  ghcr.io/argonautsystems/ic-engine:4.5.0-cpu

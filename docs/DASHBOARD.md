...
In v4.1.34 the portal is served by the bridge FastAPI app. Inside the container
...
`.xls`, `.xlsx`, and `.pdf`. That is a separate setup surface. For v4.1.34 day
...
news sentiment.
The canonical key catalogue in v4.1.34 contains these eight names:
...
- `MASSIVE_API_KEY` - fundamentals, ratios, and peer comparisons through the
  FMP/Massive path; added in v4.1.34.
- `MARKETAUX_API_KEY` - per-symbol news with sentiment; added in v4.1.34.
